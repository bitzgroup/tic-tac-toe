...
`TicTacToeGameModel`/`TicTacToeMatch`, AI difficulty strategists, `GKStateMachine` turn flow),
Phase 2 (SpriteKit board rendering & input: `MenuScene`/`GameScene`, marks as `GKEntity` +
`GKSKNodeComponent`, touch handling, win-line pulse, `en`/`ja` localization), and Phase 3 (platform
app shells presenting `MenuScene` at launch; both apps verified playable start-to-finish at all
three difficulties) are complete on both platforms, both fully unit-tested (18 tests/platform on
iOS incl. `GameScenePlaythroughTests`, 16 on Android) and visually/manually verified
(simulator/device screenshots, on-device playthroughs). See `docs/ROADMAP.md` Phase 1 for a real
OSS/Apple discrepancy the iOS-first
